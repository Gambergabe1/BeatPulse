...
function extractTokenRequest(body: unknown): ParsedTokenRequest | null {
  ...
  if (typedBody.type === "blob.upload-completed") return null;
  ...
  if (typedBody.type === "blob.generate-client-token") {
    ...
    return { pathname: pathname.trim(), multipart: Boolean(typedBody.payload?.multipart) };
  }

  if (typeof typedBody.pathname === "string" && typedBody.pathname.trim()) {
    return { pathname: typedBody.pathname.trim(), multipart: Boolean(typedBody.multipart) };
  }
...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    console.error("[api/blob/upload] could not create upload token", {
