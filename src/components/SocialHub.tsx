...
import {
  Ban, Check, Clipboard, Copy, Crown, Gamepad2, LogOut, Mail, MessageCircle,
  Plus, RefreshCw, Send, ShieldCheck, UserMinus, UserPlus, Users, X,
} from 'lucide-react';
import {
  blockPlayer,
  CommunitySongRecord,
...
type SocialView = 'friends' | 'messages' | 'multiplayer';
...
export const SocialHub: React.FC<SocialHubProps> = ({ identity, songs, onLaunchMatch }) => {
  ...
  useEffect(() => {
    ...
    startSocialSession(identity)
      .then((next) => { if (!cancelled) { setSnapshot(next); setError(null); } })
  ...
  }, [room?.id, room?.status, onLaunchMatch]);
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <button onClick={() => refresh()} disabled={loading} className="rounded-xl border border-white/10 bg-white/5 p-2.5 text-white/40 transition hover:text-white" title="Refresh social data">
      ...
      {loading && !snapshot ? (
        ...
        <div className="grid gap-5 lg:grid-cols-[1.15fr_0.85fr]">
          <section className="rounded-3xl border border-neon-purple/25 bg-gradient-to-br from-neon-purple/10 to-transparent p-6">
            ...
            <div className="mt-5 flex flex-wrap gap-3">{room.status === 'lobby' && !isHost && <button onClick={() => runAction(() => setMultiplayerReady(identity, room.id, !selfParticipant?.ready))} className={`flex-1 rounded-2xl px-5 py-3 font-black uppercase tracking-wider ${selfParticipant?.ready ? 'border border-neon-green/30 bg-neon-green/10 text-neon-green' : 'bg-white text-black'}`}>{selfParticipant?.ready ? 'Ready!' : 'Ready up'}</button>}{room.status === 'lobby' && isHost && <button disabled={!canStart || busy} onClick={() => runAction(() => startMultiplayerRoom(identity, room.id))} className="flex-1 rounded-2xl bg-neon-green px-5 py-3 font-black uppercase tracking-wider text-black disabled:bg-white/10 disabled:text-white/25">{canStart ? 'Start match' : room.participants.length < 2 ? 'Waiting for players' : 'Waiting for ready checks'}</button>}{room.status === 'results' && isHost && <button onClick={() => runAction(() => requestMultiplayerRematch(identity, room.id))} className="flex-1 rounded-2xl bg-neon-blue px-5 py-3 font-black uppercase tracking-wider text-black">Run it back</button>}<button onClick={() => runAction(() => leaveMultiplayerRoom(identity, room.id))} className="rounded-2xl border border-white/10 px-4 py-3 text-white/45 hover:bg-neon-pink/10 hover:text-neon-pink"><LogOut className="h-5 w-5" /></button></div>
          </section>
...
const PlayerName = ({ player, compact = false }: { player: SocialPlayer; compact?: boolean }) => (
  <div className="flex min-w-0 items-center gap-3"><div className={`${compact ? 'h-8 w-8' : 'h-10 w-10'} relative flex shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-neon-blue/25 to-neon-purple/25 font-display font-black`}>{player.username.charAt(0).toUpperCase()}<span className={`absolute -bottom-0.5 -right-0.5 h-2.5 w-2.5 rounded-full border-2 border-zinc-950 ${presenceStyles[player.status]}`} /></div><div className="min-w-0"><p className="truncate text-sm font-bold text-white">{player.username}</p><p className="text-[10px] uppercase tracking-wider text-white/30">{player.status === 'in-game' ? 'In a match' : player.status}</p></div></div>
);

const MessageBubble = ({ message, own, sender }: { key?: React.Key; message: SocialMessage; own: boolean; sender?: string }) => (
